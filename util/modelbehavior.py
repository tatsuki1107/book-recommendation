...
def check_over_fit(
    ...
    rec_df = rec_df[
        rec_df["book_id"].isin(set(recommended_books))
    ].groupby(
        "book_id").agg({"book_rating": ["mean", "count"]}).sort_values(
            by="book_id")
...
def probability_rated_book_info(
    rating_df: pd.DataFrame,
    book_df: pd.DataFrame,
    user_df: pd.DataFrame
) -> None:
    """過去に高評価した著者、出版社の作品がレコメンドリスト内に含まれている確率を出す関数"""
