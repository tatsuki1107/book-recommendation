...
class ModelBehavior:
    ...
    def check_over_fit(self) -> None:
        """評価された数が少なく、平均的に高評価なアイテムに過学習しているかチェック"""

        if self.user_df["mf_recommend_item"] is None:
            raise "you must run recommend method"

        recommended_books = self.user_df["mf_recommend_item"].to_list()
    ...
    def probability_rated_book_info(self) -> None:
        """過去に高評価した著者、出版社の作品がレコメンドリスト内に含まれている確率を出すメソッド"""

        high_rating_df = self.rating_df.copy()
        high_rating_df = high_rating_df[high_rating_df["book_rating"] >= 8.0]
        high_rating_df = high_rating_df.merge(self.book_df, on="book_id")
        rec_df = self.user_df.copy()
        rec_df = rec_df[["user_id", "mf_recommend_item"]]

        def convert_to_dict_and_calc_prob(column: str) -> float:
            rated_column = high_rating_df.groupby(
                'user_id').agg({column: set})[column].to_dict()

            def replace_book_id_to_column_value(x) -> Set:
                res = []
                for book_id in x:
                    if len(self.book_df[self.book_df["book_id"] == book_id]) == 0:
                        continue
                    value = self.book_df[self.book_df["book_id"] == book_id][column].to_list()[
                        0]
                    res.append(value)
            ...
            recommended_column = dict(
                zip(rec_df["user_id"], rec_df["mf_recommend_item"]))

            p = dict()
            for user_id in high_rating_df["user_id"].unique():
                p[user_id] = len(rated_column[user_id] &
                                 recommended_column[user_id]) / 5
    ...
    def plot_pca(self, qi: np.ndarray) -> None:
        ...
        cont_cumcont_ratios = pd.concat(
            [contribution_ratios, cumulative_contribution_ratios], axis=1).T
        cont_cumcont_ratios.index = [
